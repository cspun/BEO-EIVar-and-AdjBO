...
### The following codes extend noisy.optimizer function of DiceOptim package ### 

# Extensions made:
# Add new acquisition function: Expected Improvement with Variance (EIVar);
# Add stopping condition for EI and EIVar;
# Add error handling to return best found so far in case errors occured in the middle of iterations or 
# when stopping condition is hit in first iteration.


#######################################################################################

# For updating Gaussian process (GP) model at every iteration
...
source('EI.grad.R')
source('max_EI.R')
# For Expected Improvement with Variance (EIVar) --- proposed acquisition function
source('EIwithVar.R')

# Note: The original function provides 8 different acquisition functions to choose from. 
# Need to import other R scripts if want to try other acqusition functions aside from EI and EIVar (available in DiceOptim subfolder).
...
##' 
##' @export
noisy.optimizer_withStopping <- function(optim.crit, optim.param=NULL, model, n.ite, noise.var=NULL, funnoise, lower, upper, 
                                         ...
                                         nugget.LB=1e-5, estim.model=NULL, type="UK")
{
  ...
  # Create vector for storing best val at each iteration
  ybest.vec = rep(0, n.ite)
  kmmean.vec = rep(0, n.ite)
  ...
  for (i.time.steps in 1:n.ite)
  {
    ...
    #-------------------------------------------------------------------------------------------------------
    #-- Find the new observation based on the EIwithVar criterion ------------------------------------
    #-------------------------------------------------------------------------------------------------------
    
    if (optim.crit =="EIwithVar")
    {
      # Find current best
      pred <- predict(object=model, newdata=model@X, type=type, checkNames = FALSE)
      mk <- pred$mean
      sk <- pred$sd
      # For noisy observations, ybest is set as minimum of kriging/GP mean of observations so far.
      i_best = which.min(mk)
      y_best <- mk[i_best] 
      
      # Store GP mean and observed values at ybest (for debugging)
      f.value <- model@y[i_best]
      kmmean.vec[i.time.steps] = y_best
      ybest.vec[i.time.steps] = f.value
      
      # Choose new observation based on EI with Var criterion
      x.new = max_EIwithVar(lower=lower, upper=upper, GP_model=model,type=type, 
                            y.best=y_best, gamma=1)
      EIVar.xnew = EIwithVar(new_val=x.new, GP_model=model , type=type , y.best=y_best, gamma=1)
      EIVar.value = EIVar.xnew$acq_val
      EI.value = EIVar.xnew$exp_imprv
      
      # Add stopping condition
      # Check stopping condition
      if (EI.value < 1e-8)
      {
        cat("Stopping condition reached at iter ",i.time.steps," Bayes Opt terminates \n")
        if (i.time.steps==1)
        {
          all.X <- t(model@X)
          cat("length x ", length(all.X),"\n")
          all.y <- t(model@y)
          cat("length y ", length(all.y),"\n")
          all.noise.var <- t(model@noise.var)
        }
        break
      }
      
      # Compare with values at DoE
      EIVar.doe <- rep(0, 1, model@n)
      for (i in 1:model@n)
      {  EIVar.doe[i] <-  EIwithVar(new_val=model@X[i,], GP_model=model , type=type , y.best=y_best, gamma=1)$acq_val}
      i.best <- which.max(EIVar.doe)
      
      # Change new observation if DoE is better
      if (EIVar.doe[i.best] > EIVar.xnew$acq_val)
    ...
    else if (optim.crit == "reinterpolation")
    {
    ...
    else if (optim.crit =="EI.plugin")
    {
      ...
      # Add plugin with minimum of kriging, i.e. GP mean
      if (optim.param$plugin.type=="min_km_mean")
      {
        pred <- predict(object=model, newdata=model@X, type=type, checkNames = FALSE)
        mk <- pred$mean
        i_best = which.min(mk)
      ...
      ybest.vec[i.time.steps] = f.value
      kmmean.vec[i.time.steps] = plugin
    ...
    # Add error handling 
    if(exists("y.new")) rm(y.new)
    try( y.new <- funnoise(x.new) )
    # if error, return best so far
    if(!exists("y.new"))
    { 
      cat("Encounter infeasible lambda \n")
      temp1 = kmmean.vec[i.time.steps]
      temp2 = ybest.vec[i.time.steps]
      kmmean.vec[(i.time.steps+1): n.ite] = temp1
      ybest.vec[(i.time.steps+1):n.ite] = temp2
  ...
  temp1 = ybest.vec[i.time.steps]
  temp2 = kmmean.vec[i.time.steps]
  ybest.vec[i.time.steps : n.ite] =temp1
  kmmean.vec[i.time.steps : n.ite] = temp2
  ...
  ############################################################################################################
  optim.result$ybestvec = ybest.vec
  optim.result$kmmeanvec = kmmean.vec
